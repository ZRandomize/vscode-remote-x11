...
import { Client, ClientChannel, ConnectConfig, X11Details, X11Options } from 'ssh2';
...
function getXAuthCookie() {
    if (!commandExists.sync('xauth')) {
        logger.log(
            'The xauth binary could not be located. This is normal on Windows must may indicate a problem on Linux/MacOS.',
        );
        return null;
    }
    let listCommand;
